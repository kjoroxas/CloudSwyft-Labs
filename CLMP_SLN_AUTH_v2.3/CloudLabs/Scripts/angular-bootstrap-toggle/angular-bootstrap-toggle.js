  ...
  angular.module('ui.toggle', [])
    ...
    .constant('toggleConfig', {
      ...
      style: '',
        ...
        * Type: boolean
        * Default: false
        * Description: Uses the angular ng-disabled attribute to disable/enabled the toggle.
        */
      ngDisabled: false
    })
    ...
    .controller('ToggleController',
      ['$scope', '$attrs', '$interpolate', '$log', 'toggleConfig', '$toggleSuppressError',
        function ($scope, $attrs, $interpolate, $log, toggleConfig, $toggleSuppressError) {
          ...
          angular.forEach(['on', 'off', 'size', 'onstyle', 'offstyle', 'style','ngDisabled'], function (key, index) {
          ...
          this.computeStyle = function () {
            ...
            $scope.disabled = self.ngDisabled;
          };
          ...
          angular.forEach(['ngDisabled'], function (key) {
              var watch = $scope.$parent.$watch($attrs[key], function (value) {
                  if (self.ngDisabled !== value) {
                      self.ngDisabled = value;
                      self.computeStyle();
                  }
              });
              $scope.$parent.$on('$destroy', function () {
    ...
    .directive('toggle', function () {
        return {
          restrict: 'E',
          transclude: true,
          template: '<div class="toggle btn" ng-class="wrapperClass" ng-style="wrapperStyle" ng-click="onSwitch()" ng-disabled="disabled">' +
          '<div class="toggle-group" style="padding:0">' +
          '<label class="btn" ng-class="onClass" ng-disabled="disabled"></label>' +
          '<label class="btn active" ng-class="offClass" ng-disabled="disabled"></label>' +
          '<span class="btn btn-default" ng-class="handleClass" ng-disabled="disabled"></span>' +
          '</div>' +
          '</div>',
          scope: {
            bindModel: '=ngModel'
